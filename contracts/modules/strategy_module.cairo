...
from lib.index_storage import STRATEGY_strategy_registry,INDEX_num_assets,INDEX_asset_addresses
...
namespace Strategy: 
    ...
    func stake{
        ...
        if _amount == Uint256.max :
            let (local trade_amount: uint256) = index_asset_balance
        else:
            let (local trade_amount: uint256) = _amount
        end
        ...
        let (strategy_registry_address) = STRATEGY_strategy_registry.read()
        let (strategy_class_hash) = IStrategy_registry.get_strategy_logic(strategy_registry_address,_asset,_amount,_protocol)
    ...
    func set_strategy_registry{
        ...
        #Only admin
        STRATEGY_strategy_registry.write(_new_registry)
        return()
    end
