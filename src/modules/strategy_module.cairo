...
func stake{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    _amount: Uint256, _asset: felt, _protocol: felt
) -> (wrapped_amount: Uint256) {
    alloc_locals;
    ...
    let (is_remaining_amount_sufficient) = uint256_le(
        Uint256(MIN_ASSET_AMOUNT, 0), remaining_amount
    );
    ...
    let (strategy_class_hash) = IStrategy_registry.get_strategy_hash(
        strategy_registry_address, _protocol
    );
    let (local wrapped: felt) = IStrategy_registry.get_wrapped_token(
        strategy_registry_address, _asset, _protocol
    );
    ...
    let (retdata_size: felt, retdata: felt*) = library_call(
        strategy_class_hash, stake_selector, 4, call_data
    );
...
@external
func unstake{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    _amount: Uint256, _wrapped_asset: felt, _protocol: felt
    ...
    let (retdata_size: felt, retdata: felt*) = library_call(
        strategy_class_hash, unstake_selector, 3, call_data
    );

    let (_, local underlying: felt) = IStrategy_registry.get_underlying_token(
        strategy_registry_address, _wrapped_asset
    );
...
@external
func set_strategy_registry{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    _new_registry: felt
) {
